...
from ...enhanced_feature_modeling import build_feature_enhancer
...
class YoloXDetector(th.nn.Module):
    ...
    def __init__(self, model_cfg: DictConfig, ssod: bool = False):
        ...
        self.feature_enhancer = build_feature_enhancer(
            enhancer_cfg=model_cfg.get('feature_enhancer', None),
            in_stages=tuple(fpn_cfg.in_stages),
    ...
    def forward_backbone(self,
        ...
        if self.feature_enhancer is not None:
            with CudaTimer(device=x.device, timer_name="FeatureEnhancer"):
                backbone_features = self.feature_enhancer(
                    backbone_features=backbone_features,
                    ev_repr=x,
                    prev_states=previous_states)
