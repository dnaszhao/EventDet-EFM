...
@torch.inference_mode()
def pred_one_seq(model, seq, filter_box_fn, label_map, prev_t=0., show_timestamp=True):
    ...
    for i, (ev_img, pred, lbl) in \
        ...
        # This is only a rough visualization index, not an exact dataset timestamp.
        t = i * 0.05 + prev_t
        if show_timestamp:
            cv2.putText(
                ev_img,
...
def main(config: DictConfig):
    ...
    vis_sequence_length = config.get('vis_sequence_length', None)
    show_timestamp = config.get('show_timestamp', True)
    config.batch_size.eval = 1
    # Keep visualization single-process on Windows for better stability.
    config.hardware.num_workers.eval = 0
    config.dataset.test_ratio = num_video / 400 if is_gen1 else num_video / 100
    if vis_sequence_length is None:
        config.dataset.sequence_length = 640 if is_gen1 else 16
    else:
        config.dataset.sequence_length = vis_sequence_length
        if not is_gen1 and vis_sequence_length > 32:
            print(
                f'Warning: vis_sequence_length={vis_sequence_length} may require large CPU memory on Gen4. '
                f'Try 8, 10, or 16 if visualization fails.')
    ...
    for batch in tqdm(loader, desc='Event sequence'):
        with fp16_autocast():
            video, eoe, prev_t = pred_one_seq(
                module, batch, filter_boxes_fn, label_map, prev_t=prev_t, show_timestamp=show_timestamp)
