...
from callbacks.custom import get_ckpt_callbacks, get_viz_callback
...
def get_exp_name(config: DictConfig):
    ...
    # dataset
    dst_name = config.dataset.name  # Use config instead of CLI
    assert config.dataset.name in dst_name
    # model
    model_name = config.model.name  # Use config instead of CLI
...
def main(config: DictConfig):
    ...
    if len(gpus) > 1:
        ...
        strategy = None
    accelerator = 'gpu' if len(gpus) >= 1 and gpus[0] >= 0 else 'cpu'
    ...
    if SLURM_JOB_ID and os.path.isdir('/checkpoint/'):
        usr = pwd.getpwuid(os.getuid())[0] if pwd is not None else getpass.getuser()
    ...
    # Use TensorBoard logger instead of wandb to avoid login issues
    from pytorch_lightning.loggers import TensorBoardLogger
    logger = TensorBoardLogger("tb_logs", name="leod")
    # automatically detect checkpoints
    ...
    # ---------------------
    callbacks = list()
    callbacks.extend(get_ckpt_callbacks(config, ckpt_dir=ckpt_dir))
    ...
    if isinstance(logger, WandbLogger) and \
            (config.logging.train.high_dim.enable or config.logging.validation.high_dim.enable):
        viz_callback = get_viz_callback(config=config)
        callbacks.append(viz_callback)
    callbacks.append(ModelSummary(max_depth=2))

    # Skip model watching to avoid wandb issues
    ...
    trainer = pl.Trainer(
        ...
        strategy=strategy,
        sync_batchnorm=(accelerator == 'gpu' and len(gpus) > 1),
