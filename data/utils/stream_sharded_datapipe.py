...
try:
    ...
    # Check if ZipperLongest exists in torchdata
    try:
        from torchdata.datapipes.iter import ZipperLongest
    except ImportError:
        # If not available in torchdata, try torch.utils.data.datapipes
        try:
            from torch.utils.data.datapipes.iter import ZipperLongest
        except ImportError:
            # If not available, use regular Zipper as fallback
            from torch.utils.data.datapipes.iter import Zipper as ZipperLongest
except ImportError:
    # Fallback for newer versions of torchdata
    try:
        from torch.utils.data.datapipes.iter import Concater, IterableWrapper, IterDataPipe
        from torch.utils.data.datapipes.map import MapDataPipe
        # Check if ZipperLongest exists in torch.utils.data.datapipes
        try:
            from torch.utils.data.datapipes.iter import ZipperLongest
        except ImportError:
            # If not available, use regular Zipper as fallback
        ...
        from torchdata.stateful_dataloader.stateful_dataloader import IterDataPipe, MapDataPipe
        # Define aliases for other classes if needed
        from torch.utils.data.datapipes.iter.combining import ConcaterIterDataPipe as Concater
        from torch.utils.data.datapipes.iter.utils import IterableWrapperIterDataPipe as IterableWrapper
        from torch.utils.data.datapipes.iter.combining import ZipperIterDataPipe as ZipperLongest
...
class ShardedStreamingDataPipe(IterDataPipe):
    ...
    def get_zipped_stream_from_worker_datapipes(
        ...
        def _to_iter_datapipe(stream):
            if isinstance(stream, IterDataPipe):
                return stream
            if hasattr(stream, 'to_iter_datapipe'):
                return stream.to_iter_datapipe()
            if isinstance(stream, MapDataPipe):
                return IterableWrapper(range(len(stream))).map(stream.__getitem__)
            return IterableWrapper([stream])

        for idx, streams in enumerate(zipped_streams):
            processed_streams = [_to_iter_datapipe(stream) for stream in streams]
            # Pass all processed streams to Concater (now all should be IterDataPipe instances)
            zipped_streams[idx] = Concater(*processed_streams)
        ...
        try:
            ...
            zipped_streams = ZipperLongest(*zipped_streams)
    ...
    def __iter__(self):
        ...
        worker_id_stream = IterableWrapper(repeat(local_worker_id))
