...
class ObjectLabelFactory(ObjectLabelBase):
    ...
    def from_structured_array(object_labels: np.ndarray,
        ...
        np_labels = np.stack(np_labels, axis=1)  # [num_boxes, num_fields]
        torch_labels = th.tensor(np.asarray(np_labels), dtype=th.float32)
        objframe_idx_2_label_idx = th.tensor(
            np.asarray(objframe_idx_2_label_idx.astype('int64')),
            dtype=th.int64
        )  # [num_frames], the start/end frame_idx of labels
